...
### 悬崖和梯度爆炸
　　多层神经网络通常存在像悬崖一样的斜率较大区域，像这样的悬崖结果，梯度在更新上会很大程度地改变参数值，不管是从上还是从下接近悬崖，
...
　　在实践中，一般会显示衰减学习率知道第τ次迭代：<a><img src="https://latex.codecogs.com/gif.latex?\inline&space;\small&space;\epsilon_{k}=(1-\alpha)\epsilon_{0}&space;&plus;&space;\alpha\epsilon_{\tau}" title="\small \epsilon_{k}=(1-\alpha)\epsilon_{0} + \alpha\epsilon_{\tau}" /></a>，
其中<a><img src="https://latex.codecogs.com/gif.latex?\inline&space;\small&space;\alpha=\frac{k}{\tau}" title="\small \alpha=\frac{k}{\tau}" /></a>
在τ次迭代后，一般使ε保持常熟。
